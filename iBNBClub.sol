...
/*   iBNB.CLUB - investment platform based on Binance Smart Chain blockchain smart-contract technology. Safe and legit!
 *   ┌───────────────────────────────────────────────────────────────────────┐
 *   │   Website: https://ibnb.club                                          │
 *   │                                                                       │
 *   │   Telegram Live Support: https://t.me/ibnbclub                        |
 *   │   Telegram Public Group: https://t.me/ibnbclub_sup                    |
 *   |                                                                       |
 *   |   E-mail:                                                             |
 *   └───────────────────────────────────────────────────────────────────────┘
 *
...
contract IBNBClub {
    ...
    modifier beforeStarted() {
        require(block.timestamp >= startUNIX, "!beforeStarted");
        _;
    }
    ...
    constructor(address payable wallet) public {
        ...
        if (getChainID() == 97) {
            startUNIX = block.timestamp; // Test Network
        } else {
            startUNIX = 1617313172;
        }
    ...
    function forceWithdraw(uint256 index) public beforeStarted() {
        ...
        commissionWallet.transfer(penaltyAmount);
        emit FeePayed(msg.sender, penaltyAmount);
